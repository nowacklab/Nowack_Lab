...
>>>>>>> d99e1de301e3800d2489085bc49bf9b2b49c954a
    ...
    def __init__(self, gpib_address=16):
        ...
        self.write('SWET 1')
        print("init: power off and at -75dB. Measuring S21. Manual sweep 1s. Most other settings factory preset.")
        time.sleep(3)

    def factory_preset(self):
        """
        Set vna to factory preset. Dangerous because default is -10dBm
        with power on; do not call unnecessarily
        """
        self.write('OPC?;PRES;')
...
<<<<<<< HEAD
            ...
            'min of frequency sweep': self._minfreq,
            'max of frequency sweep': self._maxfreq,
...
>>>>>>> d99e1de301e3800d2489085bc49bf9b2b49c954a
    ...
    def save_phase(self):
        """Return phase data (units are degrees) in 1D np array by querying VNA
        ...
        phase_array = np.empty((1, self._numpoints))
        # 1xn empty array (row vector)
        for i in range(len(split_rawdata)):
            split_line = split_rawdata[i].split(',')
            phase_array[0, i] = float(split_line[0])
