...
# TODO: more safety precautions re: VNA source power and amplifier/squid limitations
    ...
    def savelog(self):
        """OLD Save dB data in array"""

        raise Exception("Use save_DB instead")

        self.write('FORM4')  # Prepare to output correct data format
        self.write('LOGM')  # Temporarily set VNA to log magnitude mode to enable saving log magnitude
        sleep_length = float(self.ask('SWET?'))*(self.averaging_factor+3)
        time.sleep(sleep_length)  # wait for averaging to finish
    ...
    def savephase(self):
        """OLD Save phase data in array"""

        raise Exception("Use save_phase instead")
