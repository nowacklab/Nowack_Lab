...
class ANC350():
        ...
        def __init__(self, montana):
            self.anc = PyANC350v4.Positioner() # Communication with controller

            for (i,s) in enumerate(self._stages):
                setattr(self, s, Positioner(self.anc, i, label=s)) # makes positioners x, y, and z
                pos = getattr(self, s) # shorthand for the current positioner we're intializing
...
class Positioner():
    def __init__(self, anc, num, V=45, freq=100, V_lim=70, label=None):
        '''
        Creates an Attocube positioner object.
        anc = A PyANC350v4.Positioner object that communciates with the ANC350
        num = Axis index. Axis numbers labeled on the ANC are num + 1
    ...
    @property
    def V(self):
        v = self.anc.getAmplitude(self.num)
        self._V = v
        return self._V
    ...
    @property
    def freq(self):
        f = self.anc.getFrequency(self.num)
        self._freq = f
        return self._freq
