...
table = readtable(csvfile,'Delimiter',',', 'Format', '%s%f%s%s'); %Table needed because more than just variable name and value
params = struct();
...
table = readtable(csvfile,'Delimiter',',', 'Format', '%s%s%s%s'); %Table needed because more than just variable name and value
