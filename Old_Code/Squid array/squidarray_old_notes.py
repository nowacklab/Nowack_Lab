...
class PCI100:
    ...
    def __init__(self, visaResource=None, sccAddress=255, parent=None):
        assert sccAddress > 0 and sccAddress < 256

        self._label = 'PCI100'
        self._sccAddress = int(sccAddress)
        self.setVisaResource(visaResource)
...
class PFL102:   
 # the following variables changed from custom "ParameterEnum" type
    ...
    def __init__(self, channel, interface, sccAddress=1, parent = None):
        assert channel>= 1 and channel <=8
        assert sccAddress > 0 and sccAddress < 255
        
        self.label = 'PFL102'
        self.channel = channel
        self.sccAddress = sccAddress            
        ...
        self._S_bias = 0 # formerly stage1BiasCurrent
        self._A_bias = 0 # formerly _stage2BiasCurrent
        self._S_flux = 0 # formerly _stage1OffsetCurrent
        self._A_flux = 0 # formerly _stage2OffsetCurrent
        self._offset = 0 # formerly _stage2OffsetVoltage
        
        self.S_bias_lim = 2000 # formerly _stage1BiasCurrentLimit
        self.A_bias_lim = 100 # formerly _stage2BiasCurrentLimit        
        self.S_flux_lim = 100 # formerly _stage1OffsetCurrentLimit
        self.A_flux_lim = 200 # formerly _stage2OffsetCurrentLimit   
        self.offset_lim = 9.8

        self.amplifierGain = 5040.0 #always 5040
        self._integratorCapacitor = '1.5nF' 
        self._feedbackResistor = '100kOhm'
        # self.heaterTriggered = False
 ...
 # Test signal
    ...
    def heat(self, t_heat=0.2, t_cool=10):
        ...
        # may need 0x3000 based on OpenSQUID code, but SCC manual says 0x1000 - doesn't look like I need 0x3000
        
        for param in ['S_bias','A_bias','S_flux','A_flux']:
            self.updateParam(param, True) #true will zero this parameter
        
        # Open SQUID: prepareForHeating, for 10*heatTime (send code, wait 100ms), for 10*coolTime (wait 100 ms)
        ...
        #from manual: There is a timing circuit that protects the sensor from being overheated in the case of software crash. The heater turns on for approximately 0.5 seconds and, if the timer is not restarted by a new “heater ON” command during this period, it turns off.
        
        # Should make sure to zero squid and array! Look at functions from original code
...
if __name__ == '__main__':
    ...
    pci = PCI100('COM1')
    pfl = PFL102(1, pci, 1) #channel, interface, sccAddress
    # pfl.A_bias = 00e-6
    # pfl.offset = -2.5e-3
