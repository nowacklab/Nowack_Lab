...
class Touchdown(Measurement):
    ...
    def check_touchdown(self):
        ...
        if i > self.numfit + self.start_offset:
            ...
            if self.C[i - self.numfit]-np.max(self.C) < _CAPACITANCE_THRESHOLD:
        ...
        while not self.touchdown:
            ...
            for i in range(self.numsteps):
                ...
                if self.touchdown:
                    ...
                    self.tdv = self.get_td_v()
                    Vtd = self.tdv
                    self.ax.axvline(self.tdv, color='r')
    ...
    def gridplot(self, axes):
        ...
        if self.touchdown:
            axes.axvline(self.tdv, color='r')
            axes.annotate("{0:.2f}".format(self.tdv), xy=(0.05, 0.90),
                      xycoords="axes fraction", fontsize=8)
