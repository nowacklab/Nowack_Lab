...
class zitransport():
    ...
    def __init__(self, rbias, lockin, currentinput = 0, dccurrentinput = 0,
                ...
                TAchannel = 0, voltagerange = 2, currentrange = 2,
        ...
        if freq > 200:
            tampdc = 0 #do not admit dc
            lockinac = 1 #ac couple
        elif freq > 50:
            tampdc = 0
            lockinac = 0
        else:
...
['/%s/sigins/%d/IMP50'       % (lockin.device_id, voltage), 0],
...
['/%s/sigouts/%d/enables/*'% (lockin.device_id, voltageoutput, out_mix_ch),0],
['/%s/sigouts/%d/enables/*'
                        % (lockin.device_id, voltageoutput, out_mix_ch),.01],
...
['/%s/sigouts/%d/enables/%d'% (lockin.device_id, voltageoutput, out_mix_ch),3],
        ...
        self.settler = alexsweep.Delayer(settle*self.tc)
    ...
    def dccurrent(self, value):
        ...
        self.lockin.PIDS_0_SETPOINT = TAgain*value
        self.lockin.PIDS_0_CENTER = value*(rbias + estdevres)
        self.lockin.PIDS_0_ENABLE = 1
        self.lockin.PIDS_0_DEFAULT = 0

    def RvsGate(self, vstart, vstop, numpoints, settle = 5,
        ...
        self.gatesweep = alexsweep.Sweep("RvsGate" + self.desc,
        ...
        self.gatesweep.set_points(numpoints)
        ...
        self.gatesweep.repeaters = ([bg, incomp, self.settler] +
                                                        self.genericrecorders)

    def RvsI(self,istart, istop, numpoints, settle = 5, startupwait = 10,
                                acceptablecurrenterror = 1e-9, bidir = True):
        '''
        Sets up a dc bias sweep
        '''
        self.dcbiassweep = alexsweep.Sweep("RvsI" + self.desc,
                                                saveatend = False, bi = bidir)
        biascurrents = np.linspace(vstart, vstop, numpoints)
        biases = alexsweep.Active(self, "dccurrent", 'DC Current Bias',
                                                                biascurrents)
        self.dcbiassweep.set_points(numpoints)
        ...
        self.dcbiassweep.repeaters = ([biases, incomp, self.settler] +
                                                         self.genericrecorders)
