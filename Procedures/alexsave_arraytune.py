...
class SQUID_Noise_Open_Loop(SQUID_Noise):
    def __init__(self,
        ...
        self.overview_dur = overview_dur
        self.overview_rate = overview_rate
        ...
        self.saver.append('/Vsaa', 
                            np.full((self.sbias.shape[0],
                                     self.sflux.shape[0],
                                     2,
                                     self.sample_dur*self.sample_rate
                                     ),
                                    np.nan))
        self.saver.append('/Voverview',
                            np.full((self.sbias.shape[0],
                                    3,
                                     self.overview_dur*self.overview_rate
                                     ),
                                    np.nan))
    ...
    def _localconversion(self, span=.5, dur=.1):
        '''
    ...
    def _run_const_sbias(self, i):
        for sflux,j in zip(self.sflux, range(len(self.sflux))):
            ...
            self.saver.append('/Vsaa_per_sflux', conv, slc=slice(i,j))
    ...
    def _get_overview(self, i):
        r = self.daq.monitor(['dc','saa'], 
                             self.overview_dur,
                             sample_rate=self.overview_rate)
        self.saver.append('/Voverview',
                          r['t'],
                          sllc=slice(i,0))
        self.saver.append('/Voverview',
    ...
    def run(self):
        ...
        for sbias, i in zip(self.sbias, range(len(self.sbais))):
    ...
    def __init__(self,
        ...
        self.saver.append('/Vspectrum', 
                            np.full((self.sbias.shape[0],
                                     self.num_aflux,
                                     2, # 0 time, 1 Voltage
                                     self.sample_rate*self.sample_dur,
                                     ), np.nan))
        self.saver.append('/sbias', self.sbias)
        ...
        self.saver.append('/Vsquidchar', 
                            np.full((self.sbias.shape[0],
                                         ...
                                         self.fast_dur*self.fast_rate),
                                         np.nan))
        self.saver.append('/Vfc_sweep', 
                            np.full((self.sbias.shape[0],
                                     self.num_aflux,
                                     3, # 0 time, 1 Vsrc, 2 Vmeas
                                     self.fc_rate*self.fc_Is.shape[0]
                                     ), np.nan))
...
class SQUID_Noise():
    ...
    def _minimize_attr(self, attr, channels=['saa'], evalchannel='saa',
        ...
        rec = self.daq.monitor(channels, self.fast_dur, 
                                sample_rate = self.fast_rate)
        error = self._midpoint(ret[evalchannel]) + offset
